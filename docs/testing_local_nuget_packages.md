...
To properly copy the package into it, run: `dotnet nuget push MeshSDK_v1.2.3.4.nupkg --source "$(realpath nuget)"`.

The last parameter is the absolute path to this `nuget` directory, you can pass it manually instead of using `realpath`.

If done correctly, the package will be copied into `./nuget/MeshSDK.1.2.3.4.nupkg` (notice it being automatically renamed). You can copy it yourself there too.

### Install package

Delete the existing MeshSDK packages:

* On Windows, delete `C:\Users\user\.nuget\packages\meshsdk`.

* On Linux: `rm -rf ~/.nuget/packages/meshsdk/`

If this is a new project, add the dependency on MeshSDK by adding the following part to the `.csproj` file:
```xml
<ItemGroup>
  <PackageReference Include="MeshSDK" Version="1.2.3.4" />
</ItemGroup>
```

Run the project. NuGet will automatically install your package into `~/.nuget/packages/meshsdk/1.2.3.4`.
