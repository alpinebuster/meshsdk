...
**GPU-Accelerated Architecture.** Built with speed and scalability in mind, `as-mesh` supports GPU acceleration and CUDA for high-performance computing.
...
* **On Windows:** `scripts\mrbind\generate_win.bat -B --trace` from the VS developer command prompt (use the `x64 Native` one!).
  ...
  When generating the Python bindings, the current directory matters, as this will look for `as-mesh` in `./source/x64/Release`. Add `VS_MODE=Debug` at the end if you built `as-mesh` in debug mode.
...
* **On Linux:** `make -f scripts/mrbind/generate.mk -B --trace`
  ...
  This will look for `as-mesh` in `./build/Release/bin`. Pass `MESHSDK_SHLIB_DIR=path/to/bin` for a different directory.
