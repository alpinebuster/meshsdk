...
if [ "${MR_EMSCRIPTEN}" == "ON" ]; then
  ...
  ${SCRIPT_DIR}/thirdparty/boost-libs-download.sh ${MESHSDK_THIRDPARTY_DIR}/boost-libs
  CMAKE_OPTIONS="${MR_CMAKE_OPTIONS}" ${SCRIPT_DIR}/thirdparty/boost-libs.sh ${MESHSDK_THIRDPARTY_DIR}/boost-libs
  # remove excess header files as they're distributed by Emscripten
  find ${MESHSDK_THIRDPARTY_ROOT_DIR}/include/boost -mindepth 1 -maxdepth 1 -not -name 'locale*' -exec rm -r "{}" \;
