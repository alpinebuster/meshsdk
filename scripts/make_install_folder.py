...
def prepare_includes_list():
	...
	it.append_includes_list(it.path_to_sources, skipped_dir_regexes = [re.compile('x64(/.*)?'), re.compile('TempOutput(/.*)?'), re.compile('MeshSDKC2(/.*)?'), re.compile('MeshSDKC2Cuda(/.*)?')])
	it.append_includes_list(os.path.join(it.path_to_sources, "MeshSDKC2/include"))
	it.append_includes_list(os.path.join(it.path_to_sources, "MeshSDKC2Cuda/include"))
