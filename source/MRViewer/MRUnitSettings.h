...
/// Returns the units in which the coordinates are stored in the memory of MeshSDK-based application;
/// it is computes as getModelLengthUnit().or_else( getUiLengthUnit() );
/// this function is important for serialization in the formats with known length units
[[nodiscard]] MRVIEWER_API std::optional<LengthUnit> getActualModelLengthUnit();
